 ...
 * whichname:
 *   0: use name, end at first '\0', '\r' or '\n'.
 *   1: use catalog number
 *   2: use international designator
 * if ok return 0 else return -1
 */
int
db_tle (int whichname, char *name, char *l1, char *l2, Obj *op)
{
	double ep;
	int i;

	/* skip whitespace then check for correct line numbers, matching
	 * catalog numbers and correct checksums.
	 */
	while (isspace(*l1))
	    l1++;
	if (*l1 != '1')
	...
	case 1:		/* catalog number */
	    sprintf (op->o_name, "%.5s", l1+2);
	    for (i = 4; i >= 0 && op->o_name[i] == ' '; i--)
		op->o_name[i] = '\0';
	    break;
	case 2:		/* international designator */
	    sprintf (op->o_name, "%.8s", l1+9);
	    for (i = 7; i >= 0 && op->o_name[i] == ' '; i--)
		op->o_name[i] = '\0';
	    break;
	default:	/* error */
	    return (-1);
...
enm (char *flds[MAXFLDS])
{
	...
	return (nsf > 0 ? sflds[0] : (char *) "Unknown");
}
