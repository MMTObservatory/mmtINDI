...
static const char *cns_namemap[ NCNS ] = {
   /*  0 */   "And: Andromeda",
   /*  1 */   "Ant: Antlia",
   /*  2 */   "Aps: Apus",
   /*  3 */   "Aql: Aquila",
...
static void
addFigList (ConFig **ncp, int *nused, int c, int drawcode, double ra, double dec)
{
	ConFig *cp;

	ncp[c]= (ConFig*) realloc (ncp[c], (nused[c]+1)*sizeof(ConFig));
	cp = &ncp[c][nused[c]++];
...
cns_loadfigs (FILE *fp, char *msg)
{
	...
	ConFig **ncp;			/* array of ConFig[] for each cnstn */
	int *nused;			/* number of ConFig[] for each cnstn */
	int c = -1;			/* index, same as cns_namemap[] */
	int s = 0;			/* status */

	/* init the temp lists */
	ncp = (ConFig **) calloc (NCNS, sizeof(ConFig*));
	...
	while (fgets (line, sizeof(line), fp)) {
		...
		*lp = '\0';
		...
		/* add to list */
		addFigList (ncp, nused, c, code, ra, dec);

	    } else {
		/* finish previous list, if any */
		if (c >= 0)
		    addFigList (ncp, nused, c, -1, 0.0, 0.0);
		...
		/* init its list */
		strcpy (cname, lp);
		ncp[c] = (ConFig *) malloc (1);     /* realloc seed */
	    }
	}
